...
public class AeropuertoBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         Iterator<Embarque> iterEmbarques = deletableEntity.getEmbarques().iterator();
         for (; iterEmbarques.hasNext();)
         {
            Embarque nextInEmbarques = iterEmbarques.next();
            nextInEmbarques.setAeropuerto(null);
            iterEmbarques.remove();
            this.entityManager.merge(nextInEmbarques);
         }
