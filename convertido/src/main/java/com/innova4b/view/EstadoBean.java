...
public class EstadoBean implements Serializable
{
   ...
   public String delete()
   {
      ...
      try
      {
         ...
         Iterator<Avion> iterAvions = deletableEntity.getAvions().iterator();
         for (; iterAvions.hasNext();)
         {
            Avion nextInAvions = iterAvions.next();
            nextInAvions.setEstado(null);
            iterAvions.remove();
            this.entityManager.merge(nextInAvions);
         }
   ...
   private Predicate[] getSearchPredicates(Root<Estado> root)
   {
      ...
      int idestado = this.example.getIdestado();
      if (idestado != 0)
      {
